...
if ! ls -la  ./deepin-keyring/keyrings/deepin-archive*.gpg >/dev/null 2>&1;then
    git clone https://github.com/linuxdeepin/deepin-keyring
fi

sudo apt install debootstrap
sudo ln -sf /usr/share/debootstrap/scripts/sid /usr/share/debootstrap/scripts/lion
rm -rf "$rootfs_dir"
sudo debootstrap --variant=minbase --keyring="$(readlink -f ./deepin-keyring/keyrings/deepin-archive*.gpg)" ${code_name} "$rootfs_dir" ${apt_mirror}
sudo cp -av ./deepin-keyring/keyrings/*.gpg "$rootfs_dir"/etc/apt/trusted.gpg.d/
